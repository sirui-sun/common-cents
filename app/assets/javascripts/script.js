...
/* CONTACT FORM SECTION START
   ...
   //      var timeout, email,
   //          contactForm = $('#formHide'),
   //          afterSubmit = $('#afterSubmit'),
			// nameel = $('#landing_name'),
			// emailel = $('#landing_email'),
			// msgel = $('#landing_comment'),
   //          form = $('#new_landing'),
...
function checkContactForm(email) {

    var beforeSubmit = $('#beforeSubmit');  // page element to show before submit
    var afterSubmit = $('#afterSubmit');    // page element to show after submit
    var email = $.trim(email.toLowerCase());
    if (email && /^([\w-]+(?:\.[\w-]+)*)\@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$|(\[?(\d{1,3}\.){3}\d{1,3}\]?)$/.test(email)) {
        // e-mail validated, show submit div
        beforeSubmit.hide();
        afterSubmit.show();
    } else {
        $('#contactstatus').show().html("Oops - did you mistype your e-mail?").delay(4000).fadeOut(1000);
    }
}
